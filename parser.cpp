...
void Parser::block() {
	...
	while (curr_token == PROCEDURE) {
		nextToken();
        std::cout << "\n=========" << tx->name << "======\n";
        for (auto i: tx->declared_sym) std::cout << i.name << '\n';
        std::cout << "=========\n";
		if (curr_token == Ident) {
            (*curr_sym).clear();
        ...
        std::cout << "=========\n";
            ...
            else error(2, curr_sym->name); // Procedure bi trung ten
...
void Parser::error(const int code, std::string msg) const {
    ...
    switch(code) {
        ...
        case 6: 
            std::cerr << "Khong khop void khai bao cua thu tuc " + msg + " (Tham so thu " << nParameters << " la phai la mot bien hoac mot phan tu cua mang)";
            break;
